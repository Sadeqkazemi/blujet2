...
async function main() {
  ...
  const flightPerms = await prisma.permission.findMany({
  ...
  const designCartableCount = await prisma.cartableTask.count({
  ...
  const designReferralCount = await prisma.managerReferralRecipient.count({
    where: { recipientId: designDemoEmployee.id },
  });
  if (designReferralCount === 0) {
    const designReferral = await prisma.managerReferral.create({
      data: {
        fromId: commercialManager.id,
  ...
  const designActivityCount = await prisma.auditLog.count({
