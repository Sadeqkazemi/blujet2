...
export class PricingService {
  ...
  private proposalInclude() {
    ...
    } satisfies TypeORM.FarePricingProposalInclude;
  }

  /** CEO view: pending + registered lists. */
  async listForCeo() {
    const proposals = await this.typeorm.farePricingProposal.findMany({
  ...
  async upsertProposal(
    ...
    const recentSale = await this.typeorm.booking.findFirst({
  ...
  async setLegalRate(actor: AuthenticatedUser, id: string, legalRateIrr: Irr) {
    ...
    const updated = await this.typeorm.farePricingProposal.update({
      where: { id },
      data: { legalRateIrr },
      include: this.proposalInclude(),
    });
  ...
  async register(
    ...
    const updated = await this.typeorm.farePricingProposal.updateMany({
    ...
    return this.typeorm.farePricingProposal.findUniqueOrThrow({
      where: { id },
      include: this.proposalInclude(),
    });
  }
  ...
  async runAiAnalysis(actor: AuthenticatedUser, requestId?: string) {
    const pending = await this.typeorm.farePricingProposal.findMany({
    ...
    for (const s of result.suggestions) {
      ...
      await this.typeorm.farePricingProposal.update({
