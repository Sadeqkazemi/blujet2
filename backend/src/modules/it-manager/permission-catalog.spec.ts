...
describe('phase-one employee permission catalog', () => {
  it('keeps permission keys unique within each catalog department', () => {
    const keys = PERMISSION_CATALOG.map((entry) => `${entry.dept}:${entry.key}`);
  ...
  it('contains the commercial and finance action groups requested by IT', () => {
    ...
    expect([
      ...
      'cl_rules_manage',
      'ws_manage',
    ].every((key) => commercial.has(key))).toBe(true);

    expect(
      ['ag_list', 'ag_settle', 'cr_manage', 'rp_finance', 'rp_exports', 'rf_process'].every(
        (key) => finance.has(key),
      ),
    ).toBe(true);
  });
