...
export class EmployeesService {
  ...
  async catalog() {
    const rows = await this.typeorm.permission.findMany({
  ...
  private async getEmployeeOrThrow(id: string) {
    const employee = await this.typeorm.user.findFirst({
  ...
  private async deptScopeForEmployee(
    ...
    const self = await this.typeorm.user.findUniqueOrThrow({
      where: { id: actor.id },
      select: { dept: true },
    });
    return self.dept;
  ...
  async list(actor: AuthenticatedUser, query: ListEmployeesQueryDto) {
    ...
    const employees = await this.typeorm.user.findMany({
      where: {
        role: 'EMPLOYEE',
  ...
  async create(actor: AuthenticatedUser, dto: CreateEmployeeDto) {
    ...
    const grantable = isKnownCatalogDept
      ? await this.typeorm.permission.findMany({
          where: { dept: catalogDept, key: { in: dto.permissionKeys ?? [] } },
        })
      : [];

    const passwordHash = await argon2.hash(dto.password);
    const employee = await this.typeorm.user.create({
      data: {
        role: 'EMPLOYEE',
  ...
  async get(actor: AuthenticatedUser, id: string) {
    ...
    const granted = await this.typeorm.employeePermission.findMany({
      where: { employeeId: id },
  ...
  async setPermission(
    ...
    const employee = await this.getEmployeeOrThrow(id);
    const catalogDept = catalogDeptFor(employee.dept ?? '');
    const permission = await this.typeorm.permission.findUnique({
    ...
    if (grant) {
      await this.typeorm.employeePermission.upsert({
        where: {
      ...
      await this.typeorm.employeePermission.deleteMany({
  ...
  async resetPassword(actor: AuthenticatedUser, id: string) {
    ...
    await this.typeorm.$transaction([
      ...
      this.typeorm.passwordResetEvent.create({
