...
import type { SavedBankAccount } from '../../../generated/prisma/client';
import type {
...
export class BankAccountsService {
  ...
  async listMine(user: AuthenticatedUser) {
    const rows = await this.prisma.savedBankAccount.findMany({
  ...
  async create(user: AuthenticatedUser, dto: CreateBankAccountDto) {
    ...
    const count = await this.prisma.savedBankAccount.count({
      where: { userId: user.id },
    });
    ...
    const existing = await this.prisma.savedBankAccount.findFirst({
  ...
  async update(
    ...
    const row = await this.prisma.savedBankAccount.findUnique({ where: { id } });
    ...
    if (dto.isDefault) {
      await this.prisma.$transaction([
        this.prisma.savedBankAccount.updateMany({
          where: { userId: user.id, isDefault: true },
          data: { isDefault: false },
        }),
        this.prisma.savedBankAccount.update({
          where: { id },
          data: { isDefault: true },
        }),
      ]);
    }
    const updated = await this.prisma.savedBankAccount.findUniqueOrThrow({
      where: { id },
    });
  ...
  async remove(user: AuthenticatedUser, id: string) {
    ...
    await this.prisma.savedBankAccount.delete({ where: { id } });
    if (row.isDefault) {
      const next = await this.prisma.savedBankAccount.findFirst({
        where: { userId: user.id },
        orderBy: { createdAt: 'desc' },
      });
      if (next) {
        await this.prisma.savedBankAccount.update({
          where: { id: next.id },
          data: { isDefault: true },
        });
      }
