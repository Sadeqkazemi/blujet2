...
export class IdentityVerificationService {
  ...
  async adminList() {
    const rows = await this.prisma.customerIdentityVerification.findMany({
  ...
  private async adminRequireRow(id: string) {
    const row = await this.prisma.customerIdentityVerification.findUnique({
      where: { id },
  ...
  async adminApprove(actor: AuthenticatedUser, id: string) {
    ...
    const updated = await this.prisma.customerIdentityVerification.update({
      where: { id },
