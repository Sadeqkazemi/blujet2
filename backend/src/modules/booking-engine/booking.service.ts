...
export class BookingService {
  ...
  async pay(
    ...
    if (idempotencyKey) {
      const prior = await this.prisma.payIdempotencyRecord.findUnique({
        where: { idempotencyKey },
    ...
    if (paymentMethod === 'GATEWAY') {
      const existingRecon = await this.prisma.paymentReconciliation.findFirst({
        where: { bookingId: id },
        orderBy: { createdAt: 'desc' },
      });
      if (existingRecon) {
    ...
    if (idempotencyKey) {
      await this.prisma.payIdempotencyRecord
        .create({
