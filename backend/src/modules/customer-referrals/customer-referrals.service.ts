...
export class CustomerReferralsService {
  ...
  async getDashboard(user: AuthenticatedUser) {
    ...
    const rows = await this.typeorm.customerReferral.findMany({
  ...
  async applyOnSignup(referredUserId: string, rawCode?: string): Promise<void> {
    ...
    const referrer = await this.typeorm.user.findFirst({
      where: { referralCode: code, role: 'USER', isActive: true },
      select: { id: true },
    });
    if (!referrer || referrer.id === referredUserId) return;

    const already = await this.typeorm.customerReferral.findUnique({
      where: { referredUserId },
    });
    if (already) return;

    await this.typeorm.customerReferral.create({
      data: {
