...
export class AgenciesService {
  ...
  private async commercialDetailExtras(id: string) {
    ...
    const [bookings, wsApproved, apiKeys, invoiceAggs, ledgerRows] =
      await Promise.all([
        ...
        this.prisma.agencyWebserviceRequest.findMany({
          where: { agencyId: id, status: 'APPROVED' },
          orderBy: { decidedAt: 'desc' },
          take: 10,
        }),
        this.prisma.agencyApiKey.findMany({
    ...
    let paidTotalIrr = 0;
    let unpaidTotalIrr = 0;
    for (const row of invoiceAggs) {
      const amount = row._sum.amountIrr ?? 0;
      if (row.status === 'PAID') paidTotalIrr += amount;
      else unpaidTotalIrr += amount;
    }
  ...
  async decideDocument(
    ...
    const document = await this.prisma.agencyDocument.findUnique({
      where: { id: documentId },
    });
    ...
    const updated = await this.prisma.agencyDocument.updateMany({
    ...
    return this.prisma.agencyDocument.findUniqueOrThrow({
      where: { id: documentId },
      include: {
        file: { select: { fileName: true, sizeBytes: true, mimeType: true } },
      },
