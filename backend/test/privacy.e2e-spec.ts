...
describe('Privacy / GDPR export & delete (e2e)', () => {
  ...
  it('deletes the account: anonymizes passenger PII, revokes sessions, deactivates the user', async () => {
    ...
    const passenger = await typeorm.passenger.findFirstOrThrow({
      where: { bookingId },
    });
    ...
    const refreshTokens = await typeorm.refreshToken.findMany({
    ...
    const newUser = await typeorm.user.findUniqueOrThrow({ where: { phone } });
