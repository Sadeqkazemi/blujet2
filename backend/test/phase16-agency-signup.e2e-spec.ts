...
describe('Phase 16 — agency self-registration (e2e)', () => {
  ...
  afterEach(async () => {
    ...
    if (userIds.length > 0) {
      await typeorm.agencyCreditLine.deleteMany({
        where: { agencyId: { in: userIds } },
      });
      await typeorm.agencyProfile.deleteMany({
        where: { userId: { in: userIds } },
      });
      await typeorm.user.deleteMany({ where: { id: { in: userIds } } });
    }
    await typeorm.agencyMembershipRequest.deleteMany({
      where: { phone: { startsWith: phonePrefix } },
    });
    await typeorm.agencyRequestOtp.deleteMany({
      where: { phone: { startsWith: phonePrefix } },
    });
    await app.close();
  });
  ...
  it('public OTP + submit creates a real PENDING request with no email/city/documents collected', async () => {
    ...
    const row = await typeorm.agencyMembershipRequest.findUniqueOrThrow({
