...
describe('My sessions (e2e)', () => {
  ...
  it('GET/DELETE /my/sessions — USER lists own devices; revoke other session', async () => {
    ...
    const revoked = await typeorm.refreshToken.findUnique({ where: { id: other.id } });
    expect(revoked!.revokedAt).not.toBeNull();

    const after = await typeorm.refreshToken.findMany({
  ...
  it('403 for staff; 404 when revoking another user session', async () => {
    ...
    const rows = await typeorm.refreshToken.findMany({
