...
describe('Customer account refunds (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    typeorm = setup.get(TypeORMService);
    ...
    for (const [minHoursBeforeDeparture, penaltyPct, labelFa] of rules) {
      const existing = await typeorm.refundPenaltyRule.findFirst({
        where: { minHoursBeforeDeparture },
      });
      if (existing) {
        await typeorm.refundPenaltyRule.update({
          where: { id: existing.id },
          data: { penaltyPct, labelFa },
        });
      } else {
        await typeorm.refundPenaltyRule.create({
  ...
  it('allows exactly one of two concurrent submissions for one booking', async () => {
    ...
    expect(
      await typeorm.refundRequest.count({
