...
export default function SupportConversationCenter({
  ...
  return (
    <section
      ...
      className="overflow-hidden rounded-[22px] border border-[#202a3c] bg-[#0b111d] text-[#dce5f2] shadow-[0_22px_55px_rgba(11,17,29,.2)]"
      dir={locale === 'en' ? 'ltr' : 'rtl'}
    >
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#1d2738] px-5 py-4 sm:px-7">
        <h2 className="m-0 text-base font-black text-white">{t.title}</h2>
        <button
          type="button"
          onClick={onNew}
      ...
      <div className="grid grid-cols-2 gap-2 border-b border-[#1d2738] px-4 py-4 sm:grid-cols-4 sm:px-7">
        {STATUS_ORDER.map((status) => (
          <div
            key={status}
            ...
            className={`rounded-2xl border px-3 py-3 text-center ${
              status === 'OPEN'
                ? 'border-[#315a9d] bg-[#13233d] text-[#69a0ff]'
                : 'border-[#202b3e] bg-[#101827] text-[#c5cedd]'
            }`}
          >
            <div className="text-2xl" aria-hidden="true">{STATUS_ICON[status]}</div>
            <div className="mt-1 text-[11px] font-black">{labels[status]}</div>
            <div className="font-num mt-1 text-[10px] text-[#8290a6]">{counts[status].toLocaleString(locale)}</div>
          </div>
        ))}
      </div>

      {!tickets ? (
        <div className="p-10 text-center text-xs text-[#8290a6]">…</div>
      ) : tickets.length === 0 ? (
        <div className="p-12 text-center text-sm text-[#8290a6]">{t.empty}</div>
      ) : (
        <div className="grid min-h-[470px] lg:grid-cols-[minmax(330px,0.9fr)_minmax(440px,1.35fr)]">
          <div className="border-b border-[#1d2738] lg:border-b-0 lg:border-e">
            <div className="border-b border-[#1d2738] p-4 sm:px-5">
              <input
                type="search"
                value={query}
                onChange={(event) => setQuery(event.target.value)}
                placeholder={t.search}
                className="h-11 w-full rounded-xl border border-[#27344a] bg-[#101827] px-4 text-xs text-white outline-none placeholder:text-[#66758a] focus:border-[#4f82e8]"
              />
            </div>
            <div className="grid grid-cols-[1fr_88px_90px] gap-2 border-b border-[#1d2738] px-4 py-3 text-[10px] font-bold text-[#7f8ba0] sm:grid-cols-[1fr_105px_100px_80px] sm:px-5">
              ...
              {visibleTickets.map((ticket) => {
                ...
                return (
                  <button
                    ...
                    className={`grid w-full grid-cols-[1fr_88px_90px] items-center gap-2 border-b border-[#1d2738] px-4 py-4 text-start text-[11px] transition sm:grid-cols-[1fr_105px_100px_80px] sm:px-5 ${selected ? 'bg-[#121d2f]' : 'hover:bg-[#101827]'}`}
                  >
                    <span className="truncate font-black text-[#dce5f2]">{ticket.subject}</span>
                    <span dir="ltr" className="font-num truncate text-[#8f9cb0]">#{ticket.trackingCode}</span>
                    <span className="w-fit rounded-full bg-[#123027] px-2 py-1 text-[9px] font-bold text-[#64d7ad]">{labels[ticket.status]}</span>
                    <span className="hidden rounded-lg bg-[#152136] px-2 py-1.5 text-center text-[10px] font-bold text-[#6da2ff] sm:block">← {t.view}</span>
                  </button>
                );
              })}
              {visibleTickets.length === 0 && (
                <p className="px-5 py-10 text-center text-xs text-[#8290a6]">{t.noResults}</p>
              )}
            </div>
          </div>
          ...
          {active && (
            <article className="flex min-w-0 flex-col p-4 sm:p-6" data-testid="support-ticket-detail">
              <header className="flex flex-wrap items-start justify-between gap-3 border-b border-[#1d2738] pb-4">
                <div>
                  <h3 className="m-0 text-sm font-black text-white">{active.subject}</h3>
                  <p dir="ltr" aria-label={`#${active.trackingCode}`} className="font-num mt-1 text-[10px] text-[#79879c]">
                    <span>#</span><span>{active.trackingCode.slice(0, 2)}</span><span>{active.trackingCode.slice(2)}</span>
                  </p>
                </div>
                <span className="rounded-full bg-[#123027] px-3 py-1 text-[10px] font-bold text-[#64d7ad]">{labels[active.status]}</span>
              </header>
              ...
              <div className="flex max-h-[330px] min-h-[230px] flex-col gap-4 overflow-y-auto py-5">
                {(active.conversation?.length
                  ...
                  <div
                    key={message.id}
                    className={`max-w-[88%] rounded-2xl border px-4 py-3 ${message.senderType === 'STAFF' ? 'self-start border-[#364052] bg-[#29313f]' : 'self-end border-[#274b84] bg-[#142743]'}`}
                  >
                    {message.senderLabel && <div className="mb-2 text-[10px] font-black text-[#75a8ff]">{message.senderLabel}</div>}
                    <p className="m-0 whitespace-pre-wrap text-xs leading-7 text-[#d7dfeb]">{message.body}</p>
                    {message.attachments.length > 0 && (
                      <div className="mt-3 flex flex-wrap gap-2" aria-label={t.attach}>
                        {message.attachments.map((file) => (
                          <button key={file.id} type="button" onClick={() => void downloadFile(file.id, file.fileName)} className="rounded-lg bg-[#0e1725] px-2.5 py-1.5 text-[10px] font-bold text-[#79aaff]">📎 {file.fileName}</button>
                        ))}
                      </div>
                    )}
                    <time className="font-num mt-2 block text-[9px] text-[#738196]">{formatLocaleDateTime(message.createdAt, locale)}</time>
                  </div>
                ))}
              </div>
              ...
              {active.status === 'CLOSED' ? (
                ...
                <div className="mt-auto border-t border-[#1d2738] pt-4">
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full resize-none rounded-xl border border-[#1c293b] bg-[#07111a] p-3 text-xs leading-6 text-white outline-none placeholder:text-[#66758a] focus:border-[#4f82e8]"
                  />
