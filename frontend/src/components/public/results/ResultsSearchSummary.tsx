...
export default function ResultsSearchSummary({
  ...
  const fromLabel = originCity ?? origin;
  const toLabel = destCity ?? dest;
  const paxMeta =
    locale === 'en'
      ? `${adults} passenger${adults > 1 ? 's' : ''} · ${cabinLabel}`
      : `${faDigits(adults)} مسافر · ${cabinLabel}`;
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? 11 : 15, flex: 1, minWidth: 0 }}>
          ...
          <div style={{ lineHeight: 1.45, flex: 1, minWidth: 0 }}>
            ...
            >
              <span style={{ color: '#0d2640', fontWeight: 800 }}>{fromLabel}</span>
              <span style={{ color: '#1668c4' }}>{arrow}</span>
              <span style={{ color: '#0d2640', fontWeight: 800 }}>{toLabel}</span>
              ...
              {!isMobile && (
                <span style={{ color: '#8a96a6', fontSize: 12 }} dir="ltr">
                  ({origin} {arrow} {dest})
                </span>
              )}
            </div>
