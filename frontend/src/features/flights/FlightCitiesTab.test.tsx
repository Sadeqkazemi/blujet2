...
const AIRPORTS: AirportEntry[] = [{ id: 'a1', code: 'THR', cityFa: 'تهران', tz: 'Asia/Tehran' }];
...
describe('FlightCitiesTab', () => {
  it('creates a city and refreshes the list', async () => {
    ...
    const createSpy = vi.spyOn(flightsApi, 'createAirport').mockResolvedValue(created);
    const onCreated = vi.fn();

    render(<FlightCitiesTab airports={AIRPORTS} onCreated={onCreated} onDeleted={vi.fn()} />);
    ...
    await waitFor(() => expect(createSpy).toHaveBeenCalledWith({ cityFa: 'وان', code: 'VAS' }));
