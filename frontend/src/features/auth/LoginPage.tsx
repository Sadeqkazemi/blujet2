        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              dir="ltr"
              className="h-[46px] w-full rounded-xl border border-[#e2e8f0] bg-[#f8fafc] px-3.5 ps-11 pe-10 text-right text-[13px] text-[#0f172a] outline-none focus:border-accent"
              value={password}
            ...
            <button
              type="button"
              data-testid="staff-toggle-password"
              onClick={() => setShowPassword((v) => !v)}
              aria-label={showPassword ? 'پنهان کردن رمز عبور' : 'نمایش رمز عبور'}
              aria-pressed={showPassword}
              className="absolute start-3 top-1/2 flex h-8 w-8 -translate-y-1/2 cursor-pointer items-center justify-center rounded-lg text-[#64748b] transition hover:bg-[#e2e8f0]/80 hover:text-accent"
            >
              {showPassword ? (
