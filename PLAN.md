...
- [ ] **Phase 71 — Flight approval workflow (ops → CEO → PUBLISHED)** — branch `feature/flight-approval-workflow` (local only; not merged). Extends `FlightInstance.definitionStatus` with `PENDING_OPERATIONS` / `OPERATIONS_REJECTED` / `PUBLISHED`, role `OPERATIONS_MANAGER`, `FlightReview` history, version locking, create=`DRAFT`, CEO register → `PUBLISHED` + search sellability. Stubs only for pricing-alerts / loans / outbox. Checklist: `docs/features/flight-approval-workflow.md`.
