...
Phase 13 of the TypeORM → TypeORM migration plan. Converts the remaining
...
- **`sms`**: `SmsService` (send log) + `KavenegarSmsProvider` (the real
  vendor, previously still TypeORM-based despite being wired into this
...
- **`survey`**: `SurveyService` + `survey-lifecycle.util.ts`'s
  ...
  summary. Keeps a `TypeORMService` field solely to call the shared,
  still-TypeORM-based `materializeFlownBookings()` (same precedent as
...
- **Real runtime bug, the same class caught three times this phase**:
  ...
  originally omitted `where` (mirroring TypeORM's `findFirst({orderBy})`,
...
- **`SurveyInvite.token` had no `@BeforeInsert()` default-generator** —
  the TypeORM schema's `token String @unique @default(uuid())` has no
...
- **`SurveyInvite` has no inverse relation to `SurveyResponse`** — the
  ...
  `survey.service.ts` relied on TypeORM's `include: {response: true}` /
...
- **A real, reproducible timing regression in a stress test, root-caused
  ...
  **both** the old TypeORM code and the new TypeORM code identically,
  ...
  resolves promises in a tighter chain than TypeORM's own engine, which
...
- **Tooling reminder carried over from Phase 12**: `npx typeorm migrate
  reset --force` on this TypeORM version doesn't auto-run the seed step;
  every reset in this phase was followed by an explicit `npx typeorm db
...
The TypeORM-based seed script (`backend/typeorm/seed.ts`), then the e2e
test-fixture layer (every `test/*.e2e-spec.ts` file still creates its
fixtures via a raw `TypeORMClient` — converting this is its own dedicated
phase given the number of files), then infra/CI/TypeORM removal (delete
`typeorm/` dir, `generated/typeorm/`, `TypeORMModule`, `@typeorm/*` deps,
update Dockerfile/CI to TypeORM migrations), then the final `CLAUDE.md`
update reflecting the TypeORM switch plus a migration summary doc. TypeORM
remains the active ORM for the seed script and every e2e fixture; nothing
removed until the dedicated TypeORM-removal phase.
