...
## PSS electronic ticket documents and flight coupons (2026-09-01)

Ticket issuance is a single transactional domain operation shared by public
checkout, direct agency/API sales, staff manual issuance, and managerial-lock
finalization. Each ticketed passenger receives one immutable electronic ticket
document and one coupon for the booking's current flight segment.

- Existing booking responses keep `passengers[].ticketNo` and
  `passengers[].ticketIssuedAt` for compatibility and additionally return
  `passengers[].ticketDocument` with `{ id, documentNo, status, issuedAt,
  originalIssueAt, coupons[] }`.
- Each coupon returns `{ id, sequenceNo, status, flightInstanceId, flightNo,
  originCode, destCode, departureAt, cabin, fareClassCode, fareIrr, taxIrr }`.
- Issuance is idempotent by passenger and segment. Re-reading or retrying a
  successful payment/agency/staff command cannot duplicate accountable
  documents.
- A full confirmed refund transitions the issued document and its non-final
  coupon to `REFUNDED` in the same transaction as the ledger reversal.
- DCS will later transition the coupon through `CHECKED_IN`, `BOARDED`, and
  `FLOWN`; no public endpoint may forge those operational states.

## DCS airport operations (2026-09-01)

All endpoints require `OPERATIONS_MANAGER` or `SITE_ADMIN` (an employee needs
the explicit operational permission). They operate on PSS flight coupons and
do not maintain a parallel passenger/seat source of truth.

| Method | Path | Contract |
| --- | --- | --- |
| GET | `/dcs/flights` | `date?` → departing-flight operational counters |
| GET | `/dcs/flights/:flightInstanceId` | Coupon-based passenger manifest and baggage rows |
| POST | `/dcs/coupons/:couponId/check-in` | `{ seatCode?, gate? }` → idempotent check-in and boarding-pass allocation |
| POST | `/dcs/coupons/:couponId/baggage` | `{ weightGrams, tagNo? }` → accepted accountable bag |
| POST | `/dcs/coupons/:couponId/board` | `{ gate }` → idempotent boarding scan |
| GET | `/dcs/flights/:flightInstanceId/load-summary` | Operational passenger/baggage/traffic-load totals; certified CG remains configuration-gated |

Check-in locks the coupon row and accepts only an active `OPEN` coupon in the
configured departure window. Boarding accepts only `CHECKED_IN`. Baggage is
accepted only after check-in, stored in grams, and never inferred from an
ancillary purchase. All commands are audited.
## Airline distribution gateway — Direct Connect / NDC-aligned (2026-09-01)

All endpoints use the existing `X-API-Key` authentication and agency API
scope. They expose the central PSS; they do not represent certification with a
named external GDS.

- `GET /api/v2/distribution/capabilities`
- `POST /api/v2/distribution/air-shopping`
- `POST /api/v2/distribution/offer-price`
- `POST /api/v2/distribution/orders` (`Idempotency-Key` required)
- `GET /api/v2/distribution/orders/:reference`
## Revenue Management System (2026-09-01)

- `GET /rms/portfolio`
- `GET /rms/flights/:id/control`
- `POST /rms/flights/:id/fare-rules/:ruleId/recommendation`

RMS output is advisory. Live prices and channel capacity can only be changed
through the existing role-gated fare publication/release endpoints.
